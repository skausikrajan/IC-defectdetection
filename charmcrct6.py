...
# Check if an IC is present
def is_ic_present(frame):
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    blurred = cv2.GaussianBlur(gray, (5, 5), 0)
    edges = cv2.Canny(blurred, 50, 150)

    # Count nonzero edge pixels
    edge_pixel_count = np.count_nonzero(edges)

    # If there are very few edges, assume no IC is present
    return edge_pixel_count > 500  # Threshold to detect an object
